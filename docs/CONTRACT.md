...
- POST /api/time/adjustments { kind: add|change|pto, punchId?, jobId?, jobName?, activity?, startedAt?, endedAt?, breakMinutes?, workDate?, hours?, reason, ripplingAcknowledged? } -> { adjustment } (session; `add` = missing clock, `change` = wrong job/times on an existing finished punch, `pto` = PTO request that must already be approved in Rippling — `ripplingAcknowledged: true` required; `workDate` + `hours` 0.25–24). If the office has already asked crew to approve that period, the employee’s period status becomes `changes_requested` (PTO does not block Hours are approved).
...
- POST /api/admin/punches { userId, jobId?, jobName?, activity?, costItemId?, startedAt?, endedAt?, breakMinutes?, notes?, entryKind?: clock|daily|holiday|pto, workDate?: YYYY-MM-DD, hours? } -> { punch } (admin; finished pending/approved clock, does not open a live punch. `entryKind: daily` is a lump-sum for one job on `workDate` — no clock-in/out; `hours` 0.25–24; daily hours over 6 store a 30-minute lunch break. `entryKind: holiday` is holiday pay on `workDate` — no job, no lunch, not pushed to JobTread)
