...
- GET /api/admin/hours.pdf?from=YYYY-MM-DD&to=YYYY-MM-DD -> application/pdf attachment (admin; same grouping as JSON; no watermark — the archived copy is created by **Finalized**)
...
- POST /api/admin/punches/:id/adjust { startedAt?, endedAt?, breakMinutes?, activity?, note } -> { punch, adjustment } (admin; Hours-row edit; required note 8–400 chars; writes an applied adjustment-log row, or applies a pending crew request for that clock)
