...
- POST /api/admin/punches/:id/adjust { startedAt?, endedAt?, breakMinutes?, activity?, note } -> { punch, adjustment, jtSync } (admin; Hours-row edit, including after push; required note 8–400 chars; writes an applied adjustment-log row, or applies a pending crew request for that clock; pushed clocks also update the JobTread time entry)
