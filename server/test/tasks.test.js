...
before(async () => {
  ...
  const token = await crewToken(srv.base);
  authed = withAuth(srv.base, token);
});
after(async () => { await srv.close(); });

test('tasks require a session (401 without token)', async () => {
  assert.equal((await api(srv.base, '/api/tasks?scope=today')).status, 401);
  assert.equal(
    (await api(srv.base, '/api/tasks/task_x', { method: 'PATCH', body: { progress: 1 } })).status,
    401
  );
});
...
test('scope=today returns only tasks scheduled today or overdue-incomplete', async () => {
  ...
  const { status, json } = await authed('/api/tasks?scope=today');
...
test('scope=week with weekStart returns tasks overlapping that Mon-Sun window', async () => {
  ...
  const { status, json } = await authed(`/api/tasks?scope=week&weekStart=${monday}`);
  assert.equal(status, 200);
...
test('scope=week defaults to the current week when weekStart omitted', async () => {
  const explicit = await authed(`/api/tasks?scope=week&weekStart=${mondayOf()}`);
  const implicit = await authed('/api/tasks?scope=week');
  assert.deepEqual(
...
test('invalid scope and malformed weekStart return 400', async () => {
  assert.equal((await authed('/api/tasks?scope=month')).status, 400);
  assert.equal((await authed('/api/tasks?scope=week&weekStart=07-17-2026')).status, 400);
  assert.equal((await authed('/api/tasks?scope=week&weekStart=2026-02-30')).status, 400);
  // empty scope falls back to today
  assert.equal((await authed('/api/tasks?scope=')).status, 200);
});
...
test('PATCH validation: unknown id 404, empty body 400, bad progress 400, >50 subtasks 400', async () => {
  assert.equal((await authed('/api/tasks/task_nope', { method: 'PATCH', body: { progress: 1 } })).status, 404);
  const { json } = await authed('/api/tasks?scope=week');
  const id = json.tasks[0].id;
  assert.equal((await authed(`/api/tasks/${id}`, { method: 'PATCH', body: {} })).status, 400);
  assert.equal((await authed(`/api/tasks/${id}`, { method: 'PATCH', body: { progress: 2 } })).status, 400);
  const tooMany = Array.from({ length: 51 }, (_, i) => ({ name: `s${i}`, isComplete: false }));
  assert.equal((await authed(`/api/tasks/${id}`, { method: 'PATCH', body: { subtasks: tooMany } })).status, 400);
});
